...
void* udpHandler(void *arg){
	...
	int err, sock, res, i,res2;
	unsigned int adl;
	char line[BUF_SIZE],line2[BUF_SIZE];
...
int main(int argc, char *argv[]) {
	...
	pthread_t tcpT[10], udpT[10];
	int thread_counter = 0;

	/*Create the first tcp and udp*/
    ...
    while(_resetCall){
    	...
    	sleep(20);
		if(pthread_create(&tcpT[i], NULL, tcpHandler,(void*)data)) {
	        perror("Error creating thread.");
	    } 
	   	if(pthread_create(&udpT[i], NULL, udpHandler,(void*)data)) {
        	perror("Error creating thread.");
    	}
    	
		pthread_join(tcpT[i],NULL);
		pthread_join(udpT[i],NULL);
		i++;
    }	
			...
			if(len - strlen(buffer_line) < sizeof(msg)){
			    ...
			    if(buffer_line[strlen(buffer_line) - 1] == '\n' || buffer_line[strlen(buffer_line) - 1] != EOF){
			    	if(local_file_progress >= file_progress){
				    	...
				    	//Just to better illustrate the last line
