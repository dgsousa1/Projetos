...
void* tcpHandler(void *arg){
	...
	printf("Machine verified is the central system. Waiting to receive configuration...\n");
	...
	if(_receivedCode==150) sendMessagesToServer(sock);
...
void* udpHandler(void *arg){
	...
	char* ptr = _validationMessage;//request accepted/denied
	short length=0;
	...
	generateByteMessage(version,_receivedCode,_id_machine,length,_validationMessage,final_message);

	printf("[UDP] SEND -> VERSION: %d; CODE: %d; LENGTH: %d; MACHINE ID: %d; MESSAGE: %s\n",version,_receivedCode,
		length,_id_machine,_validationMessage);
	printf("\n");
	
	/*Send message with validation code and message to the UPD client*/
