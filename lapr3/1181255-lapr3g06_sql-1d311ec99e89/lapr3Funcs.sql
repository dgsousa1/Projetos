...
--=========FUNCTION TO GET VEHICLE TYPE FROM ID================
CREATE OR REPLACE FUNCTION getVehicleTypeFromID(p_id_vehicle VARCHAR) RETURN VARCHAR
IS
    count_scooter INTEGER;
    count_bike INTEGER;
BEGIN
    SELECT count(v.id_vehicle) INTO count_scooter
    FROM trip t, scooters s, vehicles v
    WHERE v.id_vehicle = s.id_vehicle AND
          v.id_vehicle = t.id_vehicle AND
          v.id_vehicle = p_id_vehicle;
    
    IF(count_scooter>0) THEN
        RETURN('S');
        
        ELSE 
        SELECT count(v.id_vehicle) INTO count_bike
        FROM trip t, bicycles b, vehicles v
        WHERE v.id_vehicle = b.id_vehicle AND
              v.id_vehicle = t.id_vehicle AND
              v.id_vehicle = p_id_vehicle;
        IF(count_bike>0) THEN
            RETURN('B');
            ELSE
                raise_application_error(-20080, 'Vehicle invalid or not found'); 
        END IF;
    END IF;
...
--==========FUNCTION TO CHECK FREE VEHICLES PLACES==============
CREATE OR REPLACE FUNCTION checkFreeVehiclesPlaces(p_id_user VARCHAR, p_idPark VARCHAR) RETURN INTEGER
IS
    nr_free_places INTEGER;
    temp_location INTEGER;
    type_vehicle_verification VARCHAR(50);
    temp_id_vehicle VARCHAR(50);
BEGIN
    --VERIFICAR PRIMEIRO DE A LOCATION E UM PARK
    SELECT id_location INTO temp_location 
    FROM parks 
    WHERE id_park = p_idPark;
    
    --ja tenho o id do vehicle
    SELECT t.id_vehicle INTO temp_id_vehicle
    FROM trip t, finished_trip ft
    WHERE t.id_trip = ft.id_finished_trip AND  
          t.id_user = p_id_user
    ORDER BY initial_date DESC FETCH FIRST ROW ONLY;
    
    type_vehicle_verification := getVehicleTypeFromID(temp_id_vehicle);
    
    IF(type_vehicle_verification='S')THEN
        SELECT p.max_scooter-(SELECT COUNT(s.id_vehicle) 
                              FROM vehicles v, scooters s
                              WHERE v.id_location = temp_location AND 
                                    v.id_vehicle = s.id_vehicle) INTO nr_free_places
        ...
        WHERE p.id_location = temp_location;
        RETURN(nr_free_places);
    ELSE IF(type_vehicle_verification='B') THEN
...
--===========FUNCTION TO GET PARK ID FROM LOCATION=============
CREATE OR REPLACE FUNCTION getParkIdFromLocationId(p_id_location INTEGER) RETURN VARCHAR
IS
    temp_id_park VARCHAR(50);       
BEGIN
    SELECT p.id_park INTO temp_id_park
    FROM parks p
    WHERE p.id_location = p_id_location;
...
BEGIN
    ...
    FROM parks 
    WHERE id_park = p_idPark;
    dbms_output.put_line('TEMP ID LOCATION:' || temp_location_id);
    UPDATE vehicles SET 
    id_location = temp_location_id 
    WHERE id_vehicle = p_idVehicle;
    
    temp_park_id := getParkIdFromLocationId(temp_location_id);
    dbms_output.put_line('TEMP ID PARK:' || temp_park_id);
    
    SELECT t.id_trip INTO recent_trip
    FROM trip t
    WHERE t.id_vehicle = p_idVehicle
    ORDER BY t.initial_date DESC FETCH FIRST ROW ONLY;
    dbms_output.put_line('RECENTE TRIP:' || recent_trip);

    INSERT INTO finished_trip(id_finished_trip,actual_end_park,end_date) VALUES (recent_trip,temp_park_id,TO_DATE(SYSDATE,'DD/MM/YYYY HH24:MI'));
...
--=============================================================
 ...
 --=========================GET MOST RECENT FINISHED TRIP ID=================================
CREATE OR REPLACE FUNCTION getMostRecentFinishedTripID(p_idUser VARCHAR) RETURN INTEGER
IS
temp_id INTEGER;
BEGIN
SELECT ft.id_finished_trip INTO temp_id
FROM finished_trip ft , trip t
WHERE ft.id_finished_trip = t.id_trip AND t.id_user = p_idUser
ORDER BY ft.end_date DESC FETCH FIRST ROW ONLY;
RETURN (temp_id);
 EXCEPTION WHEN NO_DATA_FOUND THEN
        raise_application_error(-20066, 'Trip not found'); 
...
BEGIN
    ...
    temp_trip_id:=getMostRecentFinishedTripID(username);
   ...
   INSERT INTO INVOICE_LINE(id_invoice,"cost",trip_points,id_finished_trip) VALUES (temp_id,p_cost,a_actual_points,temp_trip_id);
...
END;
/
--=========================GET ALL BICYCLES OF PARK=================================

 CREATE OR REPLACE FUNCTION getAllBicyclesOfPark(p_latitude FLOAT,p_longitude FLOAT) RETURN SYS_REFCURSOR
IS
    v_rc SYS_REFCURSOR;
    temp_park_location INTEGER;
    t_id_park VARCHAR(10);
...
/
 ...
 --=========================GET POINTS FROM USER BY MONTH=================================
CREATE OR REPLACE FUNCTION getPointsFromUserByMonth(p_idUser VARCHAR, mon INTEGER ,yea INTEGER) RETURN INTEGER
IS
    a_points INTEGER;
    verify_user INTEGER;
BEGIN
    
    SELECT height INTO verify_user FROM "clients" WHERE id_user = p_idUser;
    
    SELECT SUM(l.trip_points) INTO a_points
    FROM invoice_line l , invoice v
    WHERE l.id_invoice = v.id_invoice AND v.id_user=p_idUser AND v."month" =mon AND v."year"=yea;
    
    IF(a_points IS NULL) THEN
    RETURN(0);
    END IF;
    RETURN(a_points);
    
       EXCEPTION WHEN NO_DATA_FOUND THEN
        raise_application_error(-20058, 'User not found'); 
  
END;
/

 --=========================GET USER TRIP REPORT BY MONTH=================================
CREATE OR REPLACE FUNCTION getUserTripReportByMonth(p_id_user VARCHAR , mon INTEGER , yea INTEGER) RETURN SYS_REFCURSOR
IS
    my_cursor SYS_REFCURSOR;
BEGIN
    OPEN my_cursor FOR  
        SELECT t.id_trip, t.id_vehicle, t.id_user,LA.latitude,LA.longitude,LA.elevation,
                                             LB.latitude,LB.longitude,LB.elevation, t.initial_date UNLOCKING_DATE, ft.end_date LOCKING_DATE, il."cost"
        FROM trip t, locations LA, locations LB, finished_trip ft , invoice_line il
        WHERE LA.id_location = (SELECT l.id_location 
                                FROM locations l, parks p1 
                                WHERE l.id_location = p1.id_location AND 
                                      p1.id_park = t.start_park) AND
              LB.id_location = (SELECT l.id_location
                                ...
                                WHERE l.id_location = p2.id_location AND 
                                      p2.id_park = ft.actual_end_park) AND 
             t.id_user = p_id_user AND t.id_trip = ft.id_finished_trip AND ft.id_finished_trip = il.id_finished_trip AND EXTRACT(MONTH FROM ft.end_date) IN (mon) 
             AND EXTRACT(YEAR FROM ft.end_date) IN (yea);
    RETURN(my_cursor);
END;
/

 --=========================GET USER REPORT BY MONTH POINTS=================================
CREATE OR REPLACE FUNCTION getUserTripReportByMonthPoints(p_id_user VARCHAR , mon INTEGER , yea INTEGER) RETURN SYS_REFCURSOR
IS
    my_cursor SYS_REFCURSOR;
BEGIN
    OPEN my_cursor FOR  
        SELECT t.id_trip, t.id_vehicle, t.id_user,LA.latitude,LA.longitude,LA.elevation,
                                             LB.latitude,LB.longitude,LB.elevation, t.initial_date UNLOCKING_DATE, ft.end_date LOCKING_DATE, il.trip_points
        FROM trip t, locations LA, locations LB, finished_trip ft , invoice_line il
        WHERE LA.id_location = (SELECT l.id_location 
                                FROM locations l, parks p1 
                                WHERE l.id_location = p1.id_location AND 
